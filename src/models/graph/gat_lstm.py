...
class GATLSTMModel(nn.Module):
    ...
    def forward(
        ...
        Optimized version using reshape instead of for loop for better GPU utilization.
        ...
        # Optimized: Reshape to (batch_size * seq_len, num_nodes, input_size)
        # Apply GAT layers in parallel for all time steps
        x_reshaped = x.view(batch_size * seq_len, num_nodes, input_size)
        
        # Apply GAT layers
        for gat_layer in self.gat_layers:
            x_reshaped = gat_layer(x_reshaped, adj_matrix)
            x_reshaped = self.dropout(x_reshaped)
        
        # Reshape back: (batch_size, seq_len, num_nodes, hidden_size)
        gat_sequence = x_reshaped.view(batch_size, seq_len, num_nodes, self.hidden_size)
...
class GATLSTMForecastModel(BaseGraphModel):
    ...
    def _build_graph_tensor_for_inference(
        ...
        """Build graph temporal tensor for inference (optimized, same format as training).
        
        This method builds a full-graph tensor (T, N, F) instead of processing
        each node separately, which significantly improves inference performance.
        
        Uses GraphDatasetBuilder for unified tensor construction.
        ...
        # Use unified GraphDatasetBuilder
        X_graph, valid_mask_graph, time_steps = GraphDatasetBuilder.build_graph_tensor(
            X=X,
            node_features=node_features,
            node_indices=node_indices,
            station_ids_array=station_ids_array,
            num_nodes=self.num_nodes,
            node_feature_size=self.node_feature_size,
            valid_mask=valid_mask
        )
        
        return X_graph, valid_mask_graph, time_steps
    ...
    def predict(
        ...
        """Make point predictions using full-graph format (optimized).
        
        Optimized version that processes all nodes at once instead of per-node loops,
        significantly improving inference performance (num_nodes times faster).
        ...
        # Build graph temporal tensor (optimized: all nodes at once)
        ...
        # Batch process (optimized: all nodes at once)
        ...
        predictions = []
        with torch.no_grad():
            output = self.model(batch_X, adj_matrix).squeeze(-1)  # (B, N)
    ...
    def predict_proba(
        ...
        """Predict probabilities using full-graph format (optimized).
