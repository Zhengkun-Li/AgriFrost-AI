"""ST-GCN (Spatial-Temporal Graph Convolutional Network) model for spatial-temporal forecasting.

ST-GCN uses separate spatial and temporal convolutions for modeling spatial-temporal patterns.
It's a classic baseline for graph-based time series forecasting.
"""
...
if not TORCH_AVAILABLE:
    raise ImportError("PyTorch is required for ST-GCN models. Please install torch.")


class SpatialGraphConvolution(nn.Module):
    """Spatial graph convolution layer.
    
    Applies graph convolution in the spatial dimension.
    """
    ...
    def forward(self, x: torch.Tensor, adj_matrix: torch.Tensor) -> torch.Tensor:
        ...
        support = torch.bmm(
            ...
            x
        )  # (batch_size, num_nodes, in_features)
        
        # Apply weight matrix: (A * X) * W
...
class TemporalConvolution(nn.Module):
    ...
    Applies 1D convolution in the temporal dimension.
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Reshape back: (batch_size, num_nodes, seq_len, out_channels)
...
class STGCNBlock(nn.Module):
    ...
    Combines spatial graph convolution and temporal convolution with residual connection.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            spatial_channels: Spatial convolution output channels.
            temporal_channels: Temporal convolution output channels.
        ...
        self.spatial_conv = SpatialGraphConvolution(in_channels, spatial_channels)
        self.spatial_bn = nn.BatchNorm1d(spatial_channels)
        
        # Temporal convolution
        self.temporal_conv = TemporalConvolution(spatial_channels, temporal_channels, kernel_size)
        
        # Residual connection
        self.residual = None
        if in_channels != temporal_channels:
            self.residual = nn.Linear(in_channels, temporal_channels)
    ...
    def forward(self, x: torch.Tensor, adj_matrix: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            Output (batch_size, num_nodes, seq_len, temporal_channels).
        """
        batch_size, num_nodes, seq_len, in_channels = x.shape
        
        # Spatial convolution (applied at each time step)
        x_spatial = []
        for t in range(seq_len):
            x_t = x[:, :, t, :]  # (batch_size, num_nodes, in_channels)
            x_t = self.spatial_conv(x_t, adj_matrix)  # (batch_size, num_nodes, spatial_channels)
            # BatchNorm expects (batch_size, channels, num_nodes) for 1D
            x_t = x_t.transpose(1, 2)  # (batch_size, spatial_channels, num_nodes)
            x_t = self.spatial_bn(x_t)
            x_t = F.relu(x_t)
            x_t = x_t.transpose(1, 2)  # (batch_size, num_nodes, spatial_channels)
            x_spatial.append(x_t)
        
        x_spatial = torch.stack(x_spatial, dim=2)  # (batch_size, num_nodes, seq_len, spatial_channels)
        
        # Temporal convolution
        x_temporal = self.temporal_conv(x_spatial)  # (batch_size, num_nodes, seq_len, temporal_channels)
        
        # Residual connection
        if self.residual is not None:
            x_residual = self.residual(x)  # (batch_size, num_nodes, seq_len, temporal_channels)
...
class STGCNModel(nn.Module):
    """ST-GCN model for spatial-temporal forecasting.
    
    Stacks multiple ST-GCN blocks for hierarchical feature learning.
    """
...
class STGCNForecastModel(BaseGraphModel):
    """ST-GCN model for frost forecasting.
    
    Wraps STGCNModel with BaseGraphModel interface and full training logic.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """Initialize ST-GCN forecast model.
        ...
        self.num_blocks = model_params.get("num_blocks", 2)
        ...
        self.dataloader_workers = int(model_params.get("dataloader_workers", default_workers))
        self.prefetch_factor = model_params.get("prefetch_factor", 4)
        self.persistent_workers = model_params.get("persistent_workers", True)
        self.log_epoch_frequency = max(1, int(model_params.get("log_epoch_frequency", 1)))
    ...
    def fit(
        ...
        **kwargs
        ...
        if log_file and not hasattr(self, 'progress_logger'):
            ...
            self.progress_logger.bind_files(brief_path=log_file, detailed_path=None)
        ...
        # Organize data by node and create sequences (reuse DCRNN logic)
        node_data_dict = {}
        ...
        for node_idx, data in node_data_dict.items():
            ...
            for i in range(len(X_node) - self.sequence_length + 1):
                X_seq = X_node[i:i + self.sequence_length]
                y_target = y_node[i + self.sequence_length - 1]
                sequences.append({
                    'X': X_seq,
                    'y': y_target,
                    'node_idx': node_idx
        ...
        # Create data loaders
        num_workers = max(0, self.dataloader_workers)
        prefetch_factor = None
        if num_workers > 0 and self.prefetch_factor:
            prefetch_factor = max(1, int(self.prefetch_factor))

        def _build_loader(dataset, shuffle: bool):
            loader_kwargs = {
            ...
            if num_workers > 0:
                ...
                loader_kwargs["persistent_workers"] = bool(self.persistent_workers)
        ...
        for epoch in range(self.epochs):
            ...
            for batch_X, batch_y, batch_nodes in train_loader:
                ...
                # Reshape: (batch_size, seq_len, num_nodes, n_features)
            ...
            if self.progress_logger and ((epoch + 1) % self.log_epoch_frequency == 0 or epoch == 0):
                self.progress_logger.log(
        ...
        # Fit calibrator if classification
    ...
    @classmethod
    def load(cls, path: Path) -> "STGCNForecastModel":
