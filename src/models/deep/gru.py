"""GRU model implementation for frost forecasting."""
...
if not TORCH_AVAILABLE:
    raise ImportError("PyTorch is required for GRU models. Please install torch.")
...
class TimeSeriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return (self.X[seq_idx], self.y[seq_idx[-1]])
...
class GRUModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # CRITICAL FIX: Use squeeze(-1) instead of squeeze() to avoid shape mismatch when batch_size=1
        # squeeze() removes all dims of size 1, causing [1, 1] -> [] (scalar) when batch=1
        # squeeze(-1) only removes the last dim, ensuring [batch_size, 1] -> [batch_size] for all batch sizes
        return output.squeeze(-1)

class GRUForecastModel(BaseModel):
    """GRU model for frost forecasting."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.use_amp = model_params.get("use_amp", False)  # Mixed precision training
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, eval_set=None, **kwargs) -> "GRUForecastModel":
        """Train the GRU model.
        ...
        is_valid, error_msg = ConfigValidator.validate_model_config("gru", self.config)
        if not is_valid:
            raise ValueError(f"Invalid GRU configuration: {error_msg}")
        ...
        is_valid, error_msg = ConfigValidator.validate_training_args(
            "gru", checkpoint_dir=checkpoint_dir
        )
        if not is_valid:
        ...
        _logger.info("🚀 Starting GRU training")
        _logger.info(f"     Device: {self.device}")
        _logger.info(f"     Input size: {self.input_size}")
        _logger.info(f"     Hidden size: {self.hidden_size}")
    ...
    def get_feature_importance(self) -> Optional[pd.DataFrame]:
        """GRU doesn't provide direct feature importance.
        
        Returns:
            None (GRU uses different importance metrics).
        """
        return None
    ...
    @classmethod
    def load(cls, path: Path) -> "GRUForecastModel":
