...
class LSTMMultiTaskModel(nn.Module):
    ...
    This model uses a shared LSTM backbone to extract temporal features,
    then branches into two task-specific heads:
    1. Temperature prediction (regression head): Predicts future temperature T(t+h)
    2. Frost probability prediction (classification head): Predicts frost occurrence probability
    
    **Multi-task Learning Design:**
    - Shared representation: Single LSTM backbone captures common temporal patterns
    - Task-specific heads: Separate linear layers for regression and classification
    - Weighted loss combination: loss_total = w_temp * temp_loss + w_frost * frost_loss
    
    **Loss Weight Selection:**
    - Default: (w_temp=0.5, w_frost=0.5) - equal contribution
    - Imbalanced frost: (w_temp=0.3, w_frost=0.7) - prioritize frost classification
    - Can be tuned via grid search or based on task importance
    
    **Advantages:**
    - Joint learning benefits both tasks through shared representation
    - More efficient than training separate models
    - Can improve generalization through regularization effect
    
    **Considerations:**
    - Temperature loss (MSE) and frost loss (BCE) have different scales
    - Weights should be tuned to balance task contributions
...
class LSTMMultiTaskForecastModel(BaseModel):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Multi-task loss weights (explicit and configurable)
        # These weights balance the contribution of temperature regression and frost classification losses
        # Default: equal weights (0.5, 0.5) - sum to 1.0 for interpretability
        # Recommended: frost_weight > temp_weight for imbalanced frost classification (e.g., 0.3, 0.7)
        # These can be tuned via grid search or based on task importance
        ...
        weight_sum = self.loss_weight_temp + self.loss_weight_frost
        if abs(weight_sum - 1.0) > 0.01:
            _logger.warning(
                f"Multi-task loss weights sum to {weight_sum:.2f} (expected ~1.0). "
                f"Consider normalizing: temp={self.loss_weight_temp/weight_sum:.2f}, "
                f"frost={self.loss_weight_frost/weight_sum:.2f}"
            )
    ...
    def fit(self, X: pd.DataFrame, y_temp: pd.Series, y_frost: pd.Series, **kwargs) -> "LSTMMultiTaskForecastModel":
        ...
        if np.isinf(X_array).any():
            _logger.warning("Found infinite values in features, clipping to finite range")
            X_array = np.clip(X_array, -1e6, 1e6)
        
        if np.isinf(y_temp_array).any() or np.isinf(y_frost_array).any():
            _logger.warning("Found infinite values in targets, clipping to finite range")
        ...
        if station_ids is not None:
            ...
            if len(station_ids) != len(X_array):
                _logger.warning(f"station_ids length ({len(station_ids)}) doesn't match X length ({len(X_array)}). Ignoring station_ids.")
                station_ids = None
        ...
        if self.config.get("model_params", {}).get("use_weighted_sampler", True):
            ...
            # Clamp sampling weight to prevent excessive imbalance (max 50:1 ratio)
            max_sampling_ratio = 50.0
            if pos_w_sampler > max_sampling_ratio:
                if self.progress_logger:
                    self.progress_logger.log(
                        f"  ⚠️  Clamping sampling weight from {pos_w_sampler:.2f} to {max_sampling_ratio:.2f} "
                        f"(max ratio {max_sampling_ratio}:1) for WeightedRandomSampler",
                        flush=True,
                        detailed=True
                    )
                pos_w_sampler = max_sampling_ratio
        ...
        # Option 3: Clamp maximum weight ratio to 50:1 to prevent excessive bias
        max_weight_ratio = 50.0  # Clamp to prevent one class from dominating
        ...
        # Clamp pos_weight to prevent excessive class imbalance in loss
        # This ensures the model doesn't completely ignore one class
        if pos_weight_val > max_weight_ratio:
            if self.progress_logger:
                self.progress_logger.log(
                    f"  ⚠️  Clamping pos_weight from {pos_weight_val:.2f} to {max_weight_ratio:.2f} "
                    f"(max ratio {max_weight_ratio}:1) to prevent excessive class imbalance",
                    flush=True,
                    detailed=True
                )
            pos_weight_val = max_weight_ratio
            weight_strategy += f" (clamped to {max_weight_ratio}:1)"
        ...
        if not self.training_history:
            try:
                ...
                for m in multitask_metrics:
                    if m not in MetricSchema.get_all_metrics():
                        MetricSchema.register_custom_metric(m)
                self.training_history = TrainingHistory(metrics=multitask_metrics, use_metric_schema=True)
            except ImportError:
                from src.models.utils import TrainingHistory
        ...
        if not self.progress_logger:
            ...
            # Bind log files if available (from setup_training_tools)
            if hasattr(self, '_log_file') or hasattr(self, '_detailed_log_file'):
                self.progress_logger.bind_files(
                    brief_path=getattr(self, '_log_file', None),
                    detailed_path=getattr(self, '_detailed_log_file', None)
                )
        ...
        _logger.info("Starting LSTM Multi-Task training")
        _logger.info(f"Device: {self.device}, Input size: {self.input_size}, Hidden size: {self.hidden_size}")
        _logger.info(f"Batch size: {batch_info}, Num workers: {num_workers}, Epochs: {self.epochs}")
        _logger.info(f"Sequence length: {effective_seq_len}, Total sequences: {n_samples:,}")
        _logger.info(f"Train sequences: {len(train_dataset):,} ({len(train_loader):,} batches)")
        _logger.info(f"Val sequences: {len(val_dataset):,} ({len(val_loader):,} batches)")
        if hasattr(self, '_pos_weight') or use_focal_loss:
            if use_focal_loss:
                _logger.info(f"Loss: Focal Loss (alpha={focal_alpha}, gamma={focal_gamma})")
            else:
                _logger.info(f"Frost pos_weight: {pos_weight_val:.2f} ({weight_strategy})")
        _logger.info(
            f"Loss weights: temp={self.loss_weight_temp:.2f}, frost={self.loss_weight_frost:.2f} "
            f"(sum={self.loss_weight_temp + self.loss_weight_frost:.2f})"
        )
        _logger.info(
            "Multi-task loss: loss_total = loss_weight_temp * temp_loss + loss_weight_frost * frost_loss"
        )
        ...
        for epoch in epoch_pbar:
            ...
            for batch_X, batch_y_temp, batch_y_frost in train_pbar:
                ...
                if np.isnan(loss_total_value) or np.isinf(loss_total_value):
                    _logger.warning(
                        f"Invalid loss value (total={loss_total_value}, temp={loss_temp_value}, frost={loss_frost_value}) "
                        f"at batch {train_batches}. This may indicate numerical instability. "
                        f"Consider: reducing learning rate, checking for NaN/inf in input data, using gradient clipping"
                    )
            ...
            if self.use_early_stopping:
                ...
                if patience_counter >= self.patience:
                    if self.progress_logger:
                        self.progress_logger.on_early_stopping(epoch + 1, self.patience)
        ...
        self.progress_logger.on_training_complete(training_time, len(self.training_history))
    ...
    def save(self, path: Path) -> None:
        ...
        if hasattr(self, "_calibrator") and self._calibrator is not None and self._calibrator.is_fitted:
            try:
                ...
                _logger.warning(f"Could not save calibrator: {e}")
        
        _logger.info(f"Multi-task LSTM model saved to {path}")
    ...
    def load(self, path: Path) -> "LSTMMultiTaskForecastModel":
        ...
        self.is_fitted = True
        _logger.info(f"Multi-task LSTM model loaded from {path}")
