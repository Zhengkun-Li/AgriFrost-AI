...
if not TORCH_AVAILABLE:
    raise ImportError("PyTorch is required for LSTM models. Please install torch.")
...
class LSTMForecastModel(BaseModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, eval_set=None, **kwargs) -> "LSTMForecastModel":
        ...
        _logger.info("🚀 Starting LSTM training")
        ...
        for epoch in epoch_pbar:
            ...
            # Record best metric
            if should_validate and avg_val_loss < best_val_loss - self.min_delta:
                self.training_history.set_best('val_loss', avg_val_loss, epoch + 1)
            ...
            if self.checkpoint_manager and self.checkpoint_manager.should_save_checkpoint(epoch + 1):
                # Ensure checkpoint_dir is set
                if self.checkpoint_manager.checkpoint_dir is None:
                    if hasattr(self, 'checkpoint_dir') and self.checkpoint_dir:
                        self.checkpoint_manager.bind_dir(self.checkpoint_dir)
                ...
                if hasattr(self.progress_logger, 'log'):
