...
from .gru import TimeSeriesDataset

class Chomp1d(nn.Module):
    """Remove padding from the right side of sequence (causal convolution).
    
    This module ensures temporal causality by removing right-side padding,
    preventing the model from seeing future information during training/prediction.
    
    Example:
        If padding adds 2 elements on the right: [x1, x2, x3, pad, pad]
        After Chomp1d: [x1, x2, x3]
    """
    ...
    def forward(self, x):
        ...
        Args:
            x: Input tensor of shape (batch, channels, sequence_length + padding).
...
class TCNModel(nn.Module):
    """Temporal Convolutional Network model with causal convolution.
    
    **Causal Convolution:**
    - Uses Chomp1d to remove padding from the right side, ensuring no future information leakage
    - Each TemporalBlock uses padding = (kernel_size-1) * dilation_size on the left,
      then Chomp1d removes the right padding to maintain causal property
    
    **Receptive Field:**
    - The receptive field R is calculated as: R = 1 + sum((kernel_size-1) * dilation_i) for all layers
    - R must be >= sequence_length to ensure the model sees the entire input sequence
    """
    ...
    def compute_receptive_field(self) -> int:
        ...
        R = 1 + sum((kernel_size - 1) * dilation_i) for all layers
        
        Returns:
            Receptive field size (number of time steps the model can "see").
        """
        receptive_field = 1  # Start with 1 (current time step)
        num_levels = len(self.num_channels)
        for i in range(num_levels):
            dilation_size = 2 ** i
            # Each TemporalBlock has 2 conv layers, both contribute to receptive field
            receptive_field += 2 * (self.kernel_size - 1) * dilation_size
...
class TCNForecastModel(BaseModel):
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, eval_set=None, **kwargs) -> "TCNForecastModel":
        ...
        # Validate receptive field (critical for time series forecasting)
        receptive_field = self.model.compute_receptive_field()
        effective_seq_len = self.sequence_length
        
        if self.progress_logger:
            self.progress_logger.log(
                f"  📊 TCN Receptive Field: {receptive_field} time steps (seq_len: {effective_seq_len})",
                flush=True,
                detailed=True
            )
        
        # Warn if receptive field is smaller than sequence length
        if receptive_field < effective_seq_len:
            warning_msg = (
                f"  ⚠️  Warning: TCN receptive field ({receptive_field}) < sequence length ({effective_seq_len}). "
                f"The model may not see the entire input sequence. "
                f"Consider increasing num_channels (more layers) or kernel_size."
            )
            if self.progress_logger:
                self.progress_logger.log(warning_msg, flush=True, detailed=True)
            else:
                import logging
            ...
            if self.progress_logger:
                self.progress_logger.log(
                    f"  ✅ TCN receptive field ({receptive_field}) >= sequence length ({effective_seq_len})",
                    flush=True,
                    detailed=True
                )
        ...
        try:
            ...
            _logger.warning("sklearn not available. StandardScaler will be None.")
            StandardScaler = None
        ...
        if self.task_type == "classification" and self.config.get("model_params", {}).get("use_weighted_sampler", True):
            ...
            # Clamp sampling weight to prevent one class from dominating all batches
            if pos_sampling_weight > max_sampling_ratio:
                if self.progress_logger:
                    self.progress_logger.log(
                        f"  ⚠️  Clamping sampling weight from {pos_sampling_weight:.2f} to {max_sampling_ratio:.2f} "
                        f"(max ratio {max_sampling_ratio}:1) for WeightedRandomSampler",
                        flush=True,
                        detailed=True
                    )
                pos_sampling_weight = max_sampling_ratio
        ...
        _logger.info("🚀 Starting TCN training")
        _logger.info(f"     Device: {self.device}")
        _logger.info(f"     Input size: {self.input_size}")
        _logger.info(f"     Num channels: {self.num_channels}")
        ...
        _logger.info(f"     Batch size: {batch_info}")
        _logger.info(f"     Num workers: {num_workers}")
        _logger.info(f"     Epochs: {self.epochs}")
        _logger.info(f"     Sequence length: {effective_seq_len}")
        _logger.info(f"     Total sequences: {n_samples:,}")
        ...
        if self.task_type == "classification" and hasattr(self, "_pos_weight"):
            _logger.info(f"     Pos weight: {self._pos_weight.item():.2f}")
        sys.stdout.flush()
        ...
        for epoch in epoch_pbar:
            ...
            self.training_history.record_epoch(
                ...
                epoch_time=epoch_time
                        ...
                        # Log epoch (event-based API)
            if self.progress_logger:
                self.progress_logger.on_epoch(
                    epoch=epoch + 1,
            ...
            if self.use_early_stopping:
                ...
                if avg_val_loss < best_val_loss - self.min_delta:
                    ...
                    if self.save_best_model:
                        ...
                        # Log improvement
                        self.progress_logger.on_metric_improved("val_loss", avg_val_loss, best_val_loss, epoch=epoch + 1)
                else:
                    patience_counter += 1
