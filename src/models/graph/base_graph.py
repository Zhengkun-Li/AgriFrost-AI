...
class BaseGraphModel(BaseModel, ABC):
    ...
    def _load_or_build_graph(self, use_cache: bool = True) -> Dict:
        ...
        if use_cache:
            ...
            if cache_path.exists():
                try:
                    ...
                    # Specific errors when loading cached graph
                    if self.progress_logger:
                        self.progress_logger.log(
                    ...
                    # Unexpected errors
                    _logger.warning(f"Unexpected error loading cached graph: {e}. Building new graph.", exc_info=True)
                    if self.progress_logger:
                        self.progress_logger.log(
                            f"  ⚠️  Failed to load cached graph: {e}, building new graph",
                            flush=True,
        ...
        if use_cache:
            cache_path = get_graph_cache_path(
                ...
                self.graph_param,
                self.graph_cache_dir,
                station_ids=self.graph_builder.station_ids if self.graph_builder else None,
                station_coords=self.graph_builder.station_coords if self.graph_builder else None
            )
            try:
                GraphBuilder.save_graph(graph, cache_path, metadata_path=None)  # Cache doesn't need metadata export
                if self.progress_logger:
                    self.progress_logger.log(
                ...
                _logger.debug(f"Failed to save graph to cache: {e}")
                if self.progress_logger:
                    self.progress_logger.log(
                ...
                # Unexpected errors
                _logger.warning(f"Unexpected error saving graph to cache: {e}", exc_info=True)
                if self.progress_logger:
                    self.progress_logger.log(
                        f"  ⚠️  Failed to save graph to cache: {e}",
                        flush=True,
    ...
    def _prepare_node_features(
        ...
        **Feature Selection Logic:**
        - Raw variables and time encoding columns are now configurable via config
        - Default values are provided for backward compatibility
        - Config keys: `raw_feature_columns` and `time_feature_columns`
        - If not provided in config, uses default lists
        ...
        # Get feature column definitions from config (configurable, not hardcoded)
        # Default values for backward compatibility
        default_time_cols = ['hour_sin', 'hour_cos', 'day_sin', 'day_cos', 
                             'month_sin', 'month_cos', 'season_sin', 'season_cos']
        
        default_raw_vars = ['Air Temp (C)', 'Relative Humidity (%)', 'Wind Speed (m/s)',
                           'Wind Direction (deg)', 'Solar Radiation (W/m²)', 'Dew Point (C)',
                           'Vapor Pressure (kPa)', 'ET0 (mm)']
        
        # Read from config if available (allows customization)
        # Config structure: config['time_feature_columns'] or config['raw_feature_columns']
        # Or in nested: config['feature_selection']['time_feature_columns']
        ...
        if isinstance(config, dict):
            # Direct config keys
            time_cols = config.get('time_feature_columns')
            raw_vars = config.get('raw_feature_columns')
            
            # Nested config path
            if time_cols is None or raw_vars is None:
                feature_selection = config.get('feature_selection', {})
                if isinstance(feature_selection, dict):
                    if time_cols is None:
                        time_cols = feature_selection.get('time_feature_columns')
                    if raw_vars is None:
                        raw_vars = feature_selection.get('raw_feature_columns')
        
        # Use defaults if not found in config
        time_cols = time_cols if time_cols is not None else default_time_cols
        raw_vars = raw_vars if raw_vars is not None else default_raw_vars
        ...
        if not isinstance(time_cols, list):
            time_cols = default_time_cols
        if not isinstance(raw_vars, list):
            raw_vars = default_raw_vars
        ...
        for col in X.columns:
            if col in raw_vars:
                ...
                # This allows flexibility for custom feature sets
                raw_features.append(col)
    ...
    def save(self, path: Path) -> None:
        ...
        if self.graph is not None:
            ...
            # Save graph with metadata export to run_metadata.json (2×2+1 framework compatibility)
            # Try to find run_metadata.json in the same directory or parent directories
            metadata_path = None
            if path.exists():
                # Look for run_metadata.json in model directory or parent
                possible_metadata_paths = [
                    path / "run_metadata.json",
                    path.parent / "run_metadata.json",
                    path.parent.parent / "run_metadata.json"
                ]
                for mp in possible_metadata_paths:
                    if mp.exists():
                        metadata_path = mp
                        break
            
            GraphBuilder.save_graph(self.graph, graph_path, metadata_path=metadata_path)
